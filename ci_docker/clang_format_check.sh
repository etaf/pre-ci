...
git diff --name-only --diff-filter=d origin/${GITHUB_BASE_REF} origin/${GITHUB_HEAD_REF}
echo [Format Checking] Checking...
git diff --name-only --diff-filter=d origin/${GITHUB_BASE_REF} origin/${GITHUB_HEAD_REF} | (grep -v "3rd_parties" | grep -E "\.(cpp|c|h|hpp)$" || true) | xargs -i sh -c "clang-format-10 {} | cmp {}"
